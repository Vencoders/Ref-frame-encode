...
from .tensorlayer.bicubic_kernel import back_projection_loss
import logging
...
class SRNTT(object):
    ...
    def train(
        ...
        if use_weight_map:
            samples_weight_map = [np.pad(np.load(files_map[i], allow_pickle=True)['weights'], ((1, 1), (1, 1)), 'edge')
                                  for i in idx]
        else:
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in xrange(num_init_epochs):
                ...
                for n_batch in xrange(num_batches):
                    ...
                    if use_weight_map:
                        batch_weights = [
                            np.pad(np.load(files_map[i], allow_pickle=True)['weights'], ((1, 1), (1, 1)), 'edge')
                            for i in sub_idx]
    ...
    def test(
        ...
        if is_original_image:
            ...
            # img_input = img_hr
            img_input = imresize(img_hr, 1.0, interp='bicubic')
            h, w, _ = img_input.shape
